...
# # Importer les éléments depuis config.py
# from config import app, db, mail, role_required, save_avis_to_json, load_avis_from_json, load_services_from_json, allowed_file, create_app
# from models import db, User, Habitat, Animal, VetRecord, Avis, Service, DailyFoodRecord
...
# # if not os.path.exists(app.config['UPLOAD_FOLDER']):
# #     os.makedirs(app.config['UPLOAD_FOLDER'])
# app.config.from_object('config.Config')  # Appliquer la configuration depuis config.py
# app.config['UPLOAD_FOLDER'] = os.path.join('static', 'uploads')
# if not os.path.exists(app.config['UPLOAD_FOLDER']):
#     os.makedirs(app.config['UPLOAD_FOLDER'])
